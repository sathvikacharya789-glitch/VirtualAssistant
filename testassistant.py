...
def shutdown_system():
    ...
    reply = take_command()
    if any(w in reply for w in ["yes", "yeah", "confirm"]):
...
def process_voice():
    ...
    while listening:
        ...
        # Remove wake word
        cmd = cmd.replace("hey assistant", "").strip()

        # If only wake word spoken
        if not cmd:
            speak("Yes?")
            cmd = take_command()
...
# ---------------------- RUN -------------------------------
console_insert("Ready. Say 'Hey Assistant shutdown system'\n")
root.mainloop()
